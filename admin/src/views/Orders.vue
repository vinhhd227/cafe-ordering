...
const commitMove = async (order, toStatus, originalStatus) => {
  ...
  try {
    ...
    order.status = originalStatus; // revert optimistic update on API failure
    errorMessage.value =
      err?.response?.data?.errors?.join(", ") || "Failed to update order.";
  } finally {
    updatingId.value = null;
...
const handleDrop = (colKey) => {
  ...
  // Cancel any existing pending move for this order
  ...
  // Delay the actual API call by 5s — giving staff time to undo
  ...
  const colLabel = colKey === "Cancelled" ? "Cancelled" : colKey;
  toast.add({
    severity: "info",
    summary: `Moved to ${colLabel}`,
