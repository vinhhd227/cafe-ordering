...
const canSave = computed(() => cart.value.length > 0 && !!selectedTableId.value && !saving.value);

const tableOptions = computed(() =>
  tables.value.map((t) => ({ label: `${t.code}`, value: t.id })),
);
...
onMounted(async () => {
  ...
  try {
    const [menuRes, tablesRes] = await Promise.all([getAdminMenu(), listTables()]);
    menuCategories.value = menuRes.data ?? [];
    tables.value = (tablesRes.data ?? []).filter((t) => t.isActive && !t.isDeleted);
  } finally {
  ...
  <section class="tw:space-y-4">
    ...
    <div class="tw:grid tw:grid-cols-1 tw:lg:grid-cols-5 tw:gap-4">
      ...
      <div :class="[appCard, cardRing, 'tw:lg:col-span-3 tw:rounded-xl tw:p-4 tw:flex tw:flex-col tw:gap-3']">
        ...
        <!-- Loading -->
        <div v-if="loadingMenu" class="tw:flex tw:justify-center tw:py-8">
