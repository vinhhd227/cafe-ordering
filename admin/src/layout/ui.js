export const inputCustom = 'app-input tw:mt-3! tw:rounded-xl! tw:border! tw:transition! tw:focus:border-primary-400! tw:focus:outline-none! tw:focus:ring-2! tw:focus:ring-primary-300/40!'
export const labelCustom = 'app-label tw:text-xs tw:uppercase tw:tracking-[0.3em]'

const maskIconCustom = 'app-text-subtle tw:hover:text-primary-400! tw:top-[65%]!'
export const passwordCustom = {
    pcinputtext: {
        root: {
            class: inputCustom
        },
    },
    unmaskIcon: {
        class: maskIconCustom
    },
    maskIcon: {
        class: maskIconCustom
    },
    clearicon:{
        class: maskIconCustom
    }

}

export const btnIcon = 'tw:w-9! tw:h-9! tw:p-0! tw:flex tw:items-center tw:justify-center'

export const appCard =  'tw:bg-white tw:dark:bg-zinc-900 tw:dark:bg-linear-to-b! tw:dark:from-slate-900! tw:dark:to-slate-900/80! tw:border! tw:border-slate-200! tw:shadow-sm! tw:dark:border-white/15! tw:dark:shadow-xl tw:dark:backdrop-blur!'
export const cardRing = 'tw:dark:ring-1! tw:dark:ring-white/10!'